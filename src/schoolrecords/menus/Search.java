...
public class Search implements Menu {
  ...
  private final ArrayList<Record> searchRecords = new ArrayList<>();
  private final Records results = new Records();

  Search(Records records, MenuControl menuControl) {
    this.searchRecords.addAll(records.getAll());
  ...
  public void runUserInput(String input) {
    for (Record record : searchRecords) {
      for (String field : record.getSearch()) {
          if (field.toUpperCase().contains(input.toUpperCase())) {
              results.add(record);
          }
      }
    }
